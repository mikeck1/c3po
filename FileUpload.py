...
# Written by Michael Kaufman 2017 c3po Independent Study
import json
import bson
...
def get_id(CSV_URL):
    csv_url = CSV_URL
    import requests  # pypenv install requests -wrong command but close google it
    import csv # To read CSV
    with requests.Session() as s:
        download = s.get(CSV_URL)
...
def get_data(myString):
    ...
    # CONSTANTS FOR READING CSV FROM URL
    regions = read_txt_to_lines('/Users/Michael/Desktop/School/c3po_final/Gatoraid Server/temp/'+myString)
    ...
    for URL in URLlist:
        ...
        count +=1
        i+=1
    print "Total objects inserted into DB: "
    print count